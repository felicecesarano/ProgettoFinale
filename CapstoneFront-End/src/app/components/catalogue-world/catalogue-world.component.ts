...
@Component({
  selector: 'app-catalogue-world',
  templateUrl: './catalogue-world.component.html',
  styleUrls: ['./catalogue-world.component.scss']
})
export class CatalogueWorldComponent implements OnInit{
  products: any[] = [];
  ...
  getProducts() {
    this.dataSrv.getProducts().subscribe(
      (data: any[]) => {
        ...
        this.products = data.filter(product => product.category === 'World');
      },
      error => {
